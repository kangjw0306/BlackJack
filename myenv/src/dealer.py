        ...
        for _ in range(self.deck_number):
            for s in self.SUITS:
                for n in self.NUMBERS:
                    self.base_deck.append(s + n)
        
        
        self.ORIGINAL_DECK = self.base_deck[:]
                    
    # Reshuffles the deck by resetting it
    def reshuffle(self):
        if len(self.base_deck) <= 0.25*len(self.ORIGINAL_DECK):
            self.base_deck = self.ORIGINAL_DECK
    
    # Starts the game by dealing cards to the player and dealer
    def deal_card(self, player):
        # Deal player cards
        for _ in range(2):
            self.hit(player, self.base_deck)
        # Deal dealer cards
        for _ in range(2):
            self.hit(self, self.base_deck)
